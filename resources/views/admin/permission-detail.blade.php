            ...
            <div class="bg-green-50 border border-green-200 rounded-lg shadow-sm mb-4 md:mb-6">
                <div class="p-4 md:p-6">
                    ...
                    <p class="text-green-700 text-sm mb-4">
                        Tunjukkan QR code ini kepada penjaga gerbang saat keluar/masuk sekolah
                    </p>
                </div>
                                    ...
                                    <div>
                                        ...
                                        <p class="text-green-800 font-medium text-xs break-words">
                                            ...
                                            @if($permission->time_end)
                                                - {{ $permission->time_end ? $permission->time_end->format('d/m/Y H:i') : '' }}
                                            @endif
                                        </p>
                                    </div>
                                </div>
                ...
                <div class="p-4 md:p-6">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <p class="font-medium break-words">{{ $permission->student && $permission->student->user ? $permission->student->user->name : '-' }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">Kelas</p>
                            <p class="font-medium">{{ $permission->student && $permission->student->class_name ? $permission->student->class_name : '-' }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">Nomor Siswa</p>
                            <p class="font-mono font-medium">{{ $permission->student && $permission->student->student_number ? $permission->student->student_number : '-' }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">Email</p>
                            <p class="text-sm break-words">{{ $permission->student && $permission->student->user ? $permission->student->user->email : '-' }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 bg-white rounded-lg shadow-sm border border-gray-200">
                <div class="p-4 md:p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        @endif
                         @if($permission->status === 'submitted' || $permission->status === 'waiting_guru')
                        <div class="flex justify-start gap-4 mt-6">
                            <form method="POST" action="{{ route('admin.permission.update-status', ['id' => $permission->permission_id]) }}" @if($permission->status === 'waiting_guru') onsubmit="return confirm('Izin belum disetujui oleh guru. Tetap setujui?')" @endif>
                                @csrf
                                @method('PATCH')
                            ...
                            <form method="POST" action="{{ route('admin.permission.update-status', ['id' => $permission->permission_id]) }}">
                                @csrf
                                @method('PATCH')
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 mt-6">
                <div class="p-4 md:p-6">
                    ...
                    <div class="space-y-3">
                        @if($permission->admin)
                            <div>
                                <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider">Diproses oleh</label>
                                <div class="mt-1 text-sm text-gray-900">{{ $permission->admin->name }}</div>
                            </div>
                        @endif
                        @if($permission->updated_at)
                            <div>
                                <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider">Waktu Diproses</label>
