...
<div class="min-h-screen bg-gray-50 p-4 sm:p-6">
    <div class="max-w-4xl mx-auto">
        ...
        <!-- QR Code Card - Only show if permission is approved and active -->
        @if($permission->status === 'approved' && !$permission->isExpired())
            <div class="bg-green-50 border border-green-200 rounded-lg shadow-sm mb-4 md:mb-6">
                <div class="p-4 md:p-6">
                    ...
                    <p class="text-green-700 text-sm mb-4">Ini adalah QR code yang dibuat otomatis dengan pembuatan izin dan instruksinya.</p>
                                ...
                                <div class="flex items-start gap-3 mb-3">
                                    ...
                                    <div>
                                        ...
                                        <ul class="text-sm text-green-700 space-y-1">
                                            <li>• Penjaga gerbang WAJIB mencocokkan data siswa (nama, kelas, nomor siswa) dengan identitas asli.</li>
                                            <li>• Pastikan status perizinan "DISETUJUI" dan "AKTIF" sebelum mengizinkan keluar/masuk.</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
        ...
        <!-- Bukti dan Timeline -->
        <div class="lg:col-span-2 bg-white rounded-lg shadow-sm border border-gray-200 mt-4">
            <div class="p-4 md:p-6">
                <div class="flex items-center gap-2 text-base md:text-lg font-medium mb-4">
                    <svg class="h-4 w-4 md:h-5 md:w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <div class="p-4 bg-gray-50 rounded-lg">
                            ...
                            @if($permission->type === 'leave' && empty($permission->time_end))
                                <div class="mt-2 p-2 bg-yellow-50 border-l-4 border-yellow-400 rounded">
                                    <span class="text-xs text-yellow-700 font-semibold">Izin tanpa batas waktu akhir (leave):</span>
                                    <span class="text-xs text-yellow-700">{{ $permission->leave_explanation }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="pt-4 border-t border-gray-200">
                        ...
                        <div class="space-y-3">
                            ...
                            @if($permission->status !== 'submitted')
                                <div class="flex items-start gap-3">
                                    <div class="w-2 h-2 rounded-full mt-2 {{ $permission->status === 'approved' ? 'bg-green-600' : 'bg-red-600' }}"></div>
                                    <div class="flex-1">
                                        <p class="text-sm md:text-base font-medium">
                                            Perizinan {{ $permission->status === 'approved' ? 'disetujui' : 'ditolak' }}
                                        </p>
                                        <p class="text-xs text-gray-500 break-words">
